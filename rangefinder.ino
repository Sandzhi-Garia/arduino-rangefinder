...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) {
    Serial.println(F("OLED not found")); //checking if the oled is working
    while (true);
  }
...
void loop() {
  ...
  if (!freeze) {
    if ((dist_filtered < 2.0) || (dist_filtered > 350.0)) {
      display.print("OutOfRange");
    } else {
